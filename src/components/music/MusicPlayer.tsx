...
export function MusicPlayer() {
  ...
  return (
    <section className="relative z-30 shrink-0 border-t" style={{borderColor:"var(--color-border)",background:"var(--color-surface)"}}>
      ...
      {expanded&&(
        <div className="absolute bottom-full left-0 right-0 md:left-auto md:right-3 md:w-[430px] max-h-[60vh] flex flex-col border rounded-t-xl shadow-2xl" style={{borderColor:"var(--color-border)",background:"var(--color-surface)"}}>
          <div className="flex items-center justify-between p-3 border-b" style={{borderColor:"var(--color-border)"}}>
            ...
            <label className="px-3 py-2 rounded text-sm cursor-pointer" style={{background:"var(--color-accent)",color:"var(--color-bg)"}}>＋ Thêm MP3<input type="file" accept="audio/mpeg,.mp3" multiple className="hidden" onChange={async(e)=>{const files=Array.from(e.target.files??[]);if(!files.length)return;try{await addMusicFiles(files);await reload();setMessage(`Đã lưu ${files.length} bài trên thiết bị.`);}catch(error){setMessage((error as Error).message);}e.target.value="";}} /></label>
          </div>
      ...
      <div className="h-16 md:h-14 px-2 md:px-4 flex items-center gap-2">
        ...
        <input aria-label="Tua bài hát" className="flex-1 min-w-12 accent-[var(--color-accent)]" type="range" min="0" max={duration||0} step="0.1" value={Math.min(currentTime,duration||0)} onChange={(e)=>{const value=Number(e.target.value);if(audioRef.current)audioRef.current.currentTime=value;setCurrentTime(value);}} />
