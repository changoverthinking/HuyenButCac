...
function NoteRow({ note, active, onClick }: { note: Note; active: boolean; onClick: () => void }) {
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      className={`note-row w-full text-left px-3 py-2 rounded-lg mb-1 transition-colors ${active ? "is-active" : ""}`}
      style={{
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <span className="note-row-title font-medium truncate" style={{ color: "var(--color-text)" }}>
          {note.pinned && <Icon name="pin" size={13} />}
          {note.title || "(Không tiêu đề)"}
        </span>
        <button
          type="button"
          className="note-favorite"
          aria-label={note.favorite ? "Bỏ yêu thích ghi chú" : "Đánh dấu yêu thích ghi chú"}
          onClick={(e) => {
            e.stopPropagation();
...
export function NoteList() {
  ...
  if (view === "trash") {
    if (trashedNotes.length === 0) {
      return <div className="note-list-empty p-4 text-sm" style={{ color: "var(--color-text-muted)" }}>Thùng rác đang trống.</div>;
    }
    return (
      <div className="note-list-scroll p-2 overflow-y-auto">
        {trashedNotes.map((note) => (
          <div key={note.id} className="p-3 rounded-lg mb-2 border" style={{ borderColor: "var(--color-border)" }}>
            ...
            <div className="flex gap-3 text-sm">
              <button className="icon-label" style={{ color: "var(--color-accent)" }} onClick={() => void restoreNote(note.id)}><Icon name="refresh" size={14} /> Khôi phục</button>
              <button className="icon-label" style={{ color: "var(--color-error)" }} onClick={() => { if (window.confirm("Xóa vĩnh viễn ghi chú này?")) void hardDeleteNote(note.id); }}><Icon name="trash" size={14} /> Xóa vĩnh viễn</button>
            </div>
          </div>
        ))}
      </div>
  ...
  if (items.length === 0) {
    return (
      <div className="note-list-empty p-4 text-sm" style={{ color: "var(--color-text-muted)" }}>
