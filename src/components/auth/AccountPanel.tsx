...
export function AccountPanel({ open, onClose, onRecoveryRequired }: { open: boolean; onClose: () => void; onRecoveryRequired?: () => void }) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape" && !document.querySelector(".image-adjust-backdrop")) onClose();
    };
  ...
  if (!open) return null;
  const tabs: Array<[AccountTab, string]> = session
    ...
    >
      ...
      <div className="relative z-[1]">
        ...
        {!recovering && <div className="account-tabs mb-4" role="tablist" aria-label="Tài khoản và Cài đặt">
          {tabs.map(([tab, label]) => <button key={tab} role="tab" aria-selected={accountTab===tab} className={accountTab===tab?"is-active":""} onClick={()=>{setAccountTab(tab);setMessage("");}}>{label}</button>)}
        </div>}

        {!cloudConfigured && accountTab !== "settings" && <div className="mb-4 rounded-xl border p-3" style={{background:"var(--color-surface-alt)",borderColor:"var(--color-warning)"}}>
        ...
        : accountTab === "settings" ? <AppearanceSettings />
        : session ? <div className="space-y-4">
            ...
            {accountTab === "security" && <div className="space-y-4">
              ...
              {vaultResetNotice && <div role={vaultResetNotice.tone === "error" ? "alert" : "status"} aria-live="polite" className="rounded-xl border p-3 text-sm" style={{ borderColor: vaultResetNotice.tone === "success" ? "var(--color-accent)" : "var(--color-error)", background: "var(--color-surface-alt)" }}>
                <div className="font-semibold">{vaultResetNotice.tone === "success" ? "✓ Đặt lại Kho thành công" : "Không thể đặt lại Kho"}</div><p className="mt-1 opacity-80">{vaultResetNotice.text}</p>
              </div>}
